...
/// <summary>
/// Pins <see cref="PostgresDatabaseIntrospector.NormalizeSequenceOptions"/>: Postgres engine defaults must fold to
...
public sealed class SequenceOptionsNormalizationTests
{
    ...
    public void Normalize_BareAscendingBigint_IsAllNull()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row())
            .ShouldBe(new SequenceOptions());

    [Fact]
    public void Normalize_BareDescendingBigint_KeepsOnlyIncrement()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(increment: -1))
    ...
    public void Normalize_BareInteger_KeepsOnlyDataType()
        ...
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(dataType: "integer"))
            .ShouldBe(new SequenceOptions(DataType: SqlType.Int));

    [Fact]
    public void Normalize_BareSmallint_KeepsOnlyDataType()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(dataType: "smallint"))
    ...
    public void Normalize_StartEqualToEffectiveMinValue_FoldsStartToNull()
        ...
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(min: 5, start: 5))
            .ShouldBe(new SequenceOptions(MinValue: 5));

    [Fact]
    public void Normalize_StartDifferentFromEffectiveMinValue_IsKept()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(min: 5, start: 20))
    ...
    public void Normalize_DescendingStartEqualToEffectiveMaxValue_FoldsStartToNull()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(increment: -2, max: -5, start: -5))
            .ShouldBe(new SequenceOptions(IncrementBy: -2, MaxValue: -5));

    [Fact]
    public void Normalize_FullyOptionedRow_PreservesEveryOption()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(
    ...
    public void Normalize_CacheAboveOne_IsKept()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(cache: 20))
            .ShouldBe(new SequenceOptions(Cache: 20));

    [Fact]
    public void Normalize_Cycle_IsKept()
        => PostgresDatabaseIntrospector.NormalizeSequenceOptions(Row(cycle: true))
