...
/// <summary>
/// Pins <see cref="PostgresDatabaseIntrospector.NormalizeProcedureArguments"/>: Postgres renders a procedure's argument
...
public sealed class ProcedureArgumentNormalizationTests
{
    ...
    public void Normalize_EmptyArgumentList_StaysEmpty()
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("")
            .ShouldBe("");

    [Fact]
    public void Normalize_SingleInArgument_StripsMode()
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("IN a integer")
            .ShouldBe("a integer");

    [Fact]
    public void Normalize_MultipleInArguments_StripsEveryMode()
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("IN a integer, IN b text")
            .ShouldBe("a integer, b text");

    [Fact]
    public void Normalize_NonDefaultModes_ArePreserved()
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("IN a integer, OUT b integer, INOUT c text, VARIADIC d integer[]")
    ...
    public void Normalize_ArgumentNamedLikeMode_IsNotMangled()
        ...
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("INDEX integer")
            .ShouldBe("INDEX integer");

    [Fact]
    public void Normalize_CommaInsideParenthesisedDefault_DoesNotSplitTheArgument()
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("IN a text DEFAULT repeat('x'::text, 3), IN b integer")
            .ShouldBe("a text DEFAULT repeat('x'::text, 3), b integer");

    [Fact]
    public void Normalize_CommaInsideQuotedDefault_DoesNotSplitTheArgument()
        => PostgresDatabaseIntrospector.NormalizeProcedureArguments("IN a text DEFAULT 'one, IN two', IN b integer")
